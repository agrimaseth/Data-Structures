...
void cost(int total)
{
	cout<<"therefore minimum cost of the spanning tree : "<<total<<endl;
}
	...
	for (int i = 1; i <= noe - 1; i++)
	{
		for (int j = 1; j <= noe - i; j++)
		{
			if (graph_edge[j][3] > graph_edge[j + 1][3])
			{
				...
				graph_edge[j][1] = graph_edge[j + 1][1];		//swaping the 1st vertex
				graph_edge[j + 1][1] = t;

				t = graph_edge[j][2];
				graph_edge[j][2] = graph_edge[j + 1][2];		//swaping 2nd vertex
				graph_edge[j + 1][2] = t;

				t = graph_edge[j][3];
				graph_edge[j][3] = graph_edge[j + 1][3];		//swapping edges
				graph_edge[j + 1][3] = t;
			}
		}
	}
